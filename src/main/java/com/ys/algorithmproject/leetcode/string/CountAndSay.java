...
public class CountAndSay {
    ...
    public static String countAndSay(int n) {
        if(n <= 0){
            throw new IllegalArgumentException("input n is less than 0");
        }
        if(n == 1){
            return "1";
        }
        ...
        // 如果是 StringBuffer ，耗时2ms，改成 StringBuilder，耗时1ms
        ...
        for (int i = 0; i < chars.length-1; i++) {
            if(chars[i+1] != chars[i]){
                ...
                if(i+1==(chars.length-1)){
                    return sb.append(count).append(chars[i]).toString();
                }
            }
        }
        // 处理最后单独一个字符的情况
        sb.append("1").append(chars[chars.length-1]);
        return sb.toString();
    }
